...
/**
 * 送信バッファサイズとソケット数を変化させ，
 * 連続データを送信するクライアント側の機能の実装
 */
public class UploadExperiment
{
	...
	// 送信バッファサイズの配列
	private static final int[] BUF_SIZE_ARRAY	= {256, 512, 1024, 2048, 4096, 8192};
	
	// 同時に利用するソケット数の配列
	private static final int[] NUM_SOCKS_ARRAY	= {1, 2, 4, 8, 16, 32, 64, 128};
	...
	public static void main(String[] args)
	{
		...
		try {
			...
			UploadExperiment client = new UploadExperiment(servAddr, servPort, clog, flog);
	 ...
	 */
	UploadExperiment(String servAddr, int servPort, ConsoleLogger clog, FileLogger flog)
	{
	...
	void execute()
	{
		...
		for (int bs = 0; bs < BUF_SIZE_ARRAY.length; bs++) {
			for (int ns = 0; ns < NUM_SOCKS_ARRAY.length; ns++) {
				...
				stdDevs_mbps[bs][ns] = calcStdDev(averages_mbps[bs][ns], throughputs_mbps);
			}
		}
		// 下記(a)～(c)をファイルへ出力する．
		// (a) 平均スループット
	...
	/**
	 * バッファサイズとソケット数を変化させ，
	 * TOTAL_SIZE バイト分のデータを TRY_COUNT 回サーバへアップロードする．
	 */
		...
		for (int i = 0; i < TRY_COUNT; i++) {
			try {
				...
				MultiThreadUploader uploader = new MultiThreadUploader(
					this.servAddr, this.servPort, TOTAL_SIZE, bufferSize, numSockets
				);
				// アップロード開始時刻を取得する．
